...
func TestResourceLimitsInDocker(t *testing.T) {
	...
	cpuLimit := "50m"
	memLimit := "96Mi"
	expectedNano, err := tinyserver.ParseCPUNano(cpuLimit)
	require.NoError(t, err)
	expectedMemBytes, err := tinyserver.ParseMemoryBytes(memLimit)
	require.NoError(t, err)
...
func dockerStatsMemUsage(t *testing.T, containerID string) (usedBytes int64, limitBytes int64) {
	...
	used, err := tinyserver.ParseMemoryBytes(usedStr)
	require.NoError(t, err, "failed to parse used memory from %q", usedStr)
	limit, err := tinyserver.ParseMemoryBytes(limitStr)
...
func getFunctionFromSystemList(t *testing.T, baseURL string, name string) tinyserver.FunctionConfig {
	t.Helper()

	body := testutil.TinyFaaSSystemList(t, baseURL)
	out := make([]tinyserver.FunctionConfig, 0)
	...
	return tinyserver.FunctionConfig{}
}
